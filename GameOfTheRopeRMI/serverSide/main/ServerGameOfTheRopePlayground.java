...
public class ServerGameOfTheRopePlayground {
    ...
    public static void main(String[] args) {
        ...
        Playground playground = new Playground(reposStub);              // playground object
        IPlayground playgroundStub = null;                      // remote reference to the playground object

        try {
            playgroundStub = (IPlayground) UnicastRemoteObject.exportObject(playground, portNumb);
        } catch (RemoteException e) {
        ...
        String nameEntryObject = "Playground";                         // public name of the playground object
        ...
        try {
            while (!end)
                synchronized (Class.forName("serverSide.main.ServerGameOfTheRopePlayground")) {
                    try {
                        (Class.forName("serverSide.main.ServerGameOfTheRopePlayground")).wait();
                    } catch (InterruptedException e) {
                        GenericIO.writelnString("Playground main thread was interrupted!");
                    }
                }
        } catch (ClassNotFoundException e) {
            GenericIO.writelnString("The data type ServerGameOfTheRopePlayground was not found (blocking)!");
            e.printStackTrace();
            System.exit(1);
        }
        ...
        boolean shutdownDone = false;                                  // flag signalling the shutdown of the playground service
    ...
    public static void shutdown() {
        ...
        try {
            synchronized (Class.forName("serverSide.main.ServerGameOfTheRopePlayground")) {
                (Class.forName("serverSide.main.ServerGameOfTheRopePlayground")).notify();
            }
        } catch (ClassNotFoundException e) {
            GenericIO.writelnString("The data type ServerGameOfTheRopePlayground was not found (waking up)!");
            e.printStackTrace();
            System.exit(1);
        }
    }
